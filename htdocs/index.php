...
function renderQrModalTrigger($contentItem)
{
    if (isset($contentItem["imageQr"]) && !empty($contentItem["imageQr"]) > 0) {
        echo '<button class="tile__button js-modal-trigger" data-target="#' .
            generateId($contentItem["imageQr"]) .
            '">&#9635;</button>';
    }
}
...
echo isset($manifestUrl) && isset($isLocalHost)
...
<head>
  ...
  echo isset($projectDescription)
      ? '<meta name="description" content="' . $projectDescription . '"/>'
      : false;
  echo isset($projectKeywords)
      ? '<meta name="keywords" content="' . $projectKeywords . '"/>'
      : false;
  echo isset($projectLanguage)
      ? '<meta name="language" content="' . $projectLanguage . '"/>'
      : false;
  ?>
  ...
  <?php if (
      isset($wallpaper) &&
      !empty($wallpaper) &&
      isset($wallpaperPreload) &&
      $wallpaperPreload === true
  ) { ?>
      ...
      <ul class="list-tiles">
        <?php foreach ($key as $contentItem): ?>
          <li class="tile-container">
            <?php if (
                isset($contentItem["modal"]) &&
                count($contentItem["modal"]) > 0
            ) { ?>
              <a href="<?= $contentItem[
                  "url"
              ] ?>" rel="noopener" class="tile js-modal-trigger" data-target="#<?= generateId(
    $contentItem["title"]
) ?>">
                <?= returnImage(
                    $contentItem["image"],
            ...
            <?php } else { ?>
              <a href="<?= $contentItem[
                  "url"
              ] ?>" target="<?= $linktarget ?>" rel="noopener" class="tile">
                <?= returnImage(
                    $contentItem["image"],
                    $contentItem["title"],
                    "tile-image"
          ...
          <ul class="list-vertical">
            <?php foreach ($contentItems as $contentItem): ?>
              <li class="list-vertical__item">
                <a href="<?= $contentItem[
                    "url"
                ] ?>" target="<?= $linktarget ?>" class="list-vertical__link"><?= $contentItem[
    "title"
] ?></a>
              </li>
            <?php endforeach; ?>
      ...
      <div id="<?= generateId($contentItem["title"]) ?>" class="modal">
        <div class="modal-overlay">
          <div class="modal-header">
            <?= $contentItem["title"] ?>
            <button class="modal-header__close js-modal-trigger" data-target="#<?= generateId(
                $contentItem["title"]
            ) ?>">&times;</button>
          </div>
            ...
            <ul class="modal-list">
              <?php foreach ($contentItem["modal"] as $contentModalLink): ?>
                <li class="modal-list__item">
                  <a href="<?= $contentModalLink[
                      "url"
                  ] ?>" target="<?= $linktarget ?>" class="modal-list__link"><?= $contentModalLink[
    "title"
] ?></a>
                </li>
              <?php endforeach; ?>
        ...
        <div class="backdrop js-modal-trigger" data-target="#<?= generateId(
            $contentItem["title"]
        ) ?>"></div>
      </div>
    <?php } ?>
    <?php if (
        isset($contentItem["imageQr"]) &&
        !empty($contentItem["imageQr"])
    ) { ?>
...
<footer id="application-footer">
  <?php if (count($footer) > 0) { ?>
    <?php foreach ($footer["description"] as $contentItem): ?>
      <div class="description"><a href="<?= $contentItem[
          "url"
      ] ?>" target="<?= $linktarget ?>"><?= $contentItem["title"] ?></a></div>
    <?php endforeach; ?>
    <div class="social-profiles">
      <ul class="list-horizontal">
        <?php foreach ($footer["links"] as $contentItem): ?>
          <li><a href="<?= $contentItem[
              "url"
          ] ?>" target="<?= $linktarget ?>"><?= $contentItem[
    "title"
] ?></a></li>
        <?php endforeach; ?>
      </ul>
...
<script>
  ...
  <?php if (
      isset($useServiceWorker) &&
      !empty($useServiceWorker) &&
